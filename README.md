...
- `src/install.sh`
  - `/usr/local` 配下へ配置する最小 installer
- `scripts/build-release-assets.sh`
  - GitHub Releases 用の asset を生成する内部スクリプト
- `src/share/nas-mount-helper.desktop`
  - アプリランチャー雛形
...
配置後はアプリ一覧から `NAS Mount Helper` を起動できます。
...
2. `scripts/build-release-assets.sh` が `install.sh` と `package.tar.gz` を生成
3. workflow が `gh release create` で release asset を公開
4. 利用者側の `install.sh` が `releases/latest/download/package.tar.gz` を取得
5. 必要なら `sudo ./install.sh` で `/usr/local` 配下へ配置
